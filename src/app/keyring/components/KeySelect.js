    ...
    return (
      <div className="keySelect dropdown">
        ...
        <ul className="dropdown-menu">
          ...
          {this.props.keys.length > 1 && <li role="separator" className="divider"></li>}
          {this.props.keys.length > 1 && this.props.keys.filter((key, index) => index > 0).map((key, index) =>
            <li key={index + 1} className="text-left"><a onClick={() => this.handleClick(index + 1)}>{l10n.map.keygrid_subkeys} <span className="margin-left-md text-muted"><em>{key.keyId}</em></span></a></li>
          )}
        </ul>
      </div>
    );
