      ...
      <div>
        {
          ...
          <div className="clearfix">
            <p style={{'lineHeight': '32px', 'display': 'inline-block'}}>Welchen Schlüssel möchtest du exportieren?</p>
        ...
        <div className="form-group">
          <textarea ref={node => this.textarea = node} style={{'resize': 'none', 'overflow': 'hidden', 'backgroundColor': '#FFF'}} id="armoredKey" className="form-control" rows="13" value={armoredExport} spellCheck="false" autoComplete="off" readOnly></textarea>
        </div>
        {
          this.state.type !== 'pub' &&
        ...
        <div className="clearfix">
          <button type="button" className="btn btn-default" onClick={this.handleClickExport}>{l10n.map.key_export_create_file}</button>
          <button type="button" className="margin-left-sm btn btn-default" onClick={this.handleCopyToClipboard}>In die Zwischenablage kopieren</button>
          <button type="button" className="pull-right btn btn-default" onClick={this.props.onClose} data-dismiss="modal">{l10n.map.dialog_popup_close}</button>
